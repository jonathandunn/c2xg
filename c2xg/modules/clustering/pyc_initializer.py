...
@authors Andrei Novikov, Aleksey Kukushkin
@date 2014-2017
@copyright GNU Public License

@Updated for @jitclass by Jonathan Dunn
...
spec = [
	...
	("last_val", float64),
	("prev_value", float64),
	("sum_for_normalize", float64),
	("distance2", float64),
	("distance3", float64),
	("ret_idx", int64),
	("current_centers", int64),
	]
...
class kmeans_plusplus_initializer:
	...
	def euclidean_distance(self, a, b):
		...
		distance2 = self.euclidean_distance_sqrt(a, b)
		return distance2**(0.5)
		
	def euclidean_distance_sqrt(self, a, b):
		"""!
	...
	def __calc_distance_to_nearest_center(self):
		"""!
		...
		for j in range(self.__data.shape[0]):
			...
			#Get current point
			_point = self.__data[j,]

			# Min dist to nearest center
			min_dist = float(999999999999999.0)

			# For each center
			for i in range(0, self.current_centers):
		
				_center = self.centers[i,]
				temp_distance = self.euclidean_distance(_center, _point)
				min_dist = min(min_dist, temp_distance)
				
			#Add to list
			distance_data2.append(min_dist)

		return np.array(distance_data2)
	...
	def initialize(self):
		...
		self.centers = np.zeros(shape = (self.__amount, self.__data.shape[1]), dtype = np.float64)
		self.centers[0,] = self.__get_first_center()
		...
		for _ in range(1, self.__amount):
			...
			distance_data = self.__calc_distance_to_nearest_center()
			...
			# Add new center
			self.centers[self.current_centers,] = self.__data[ret_idx]
			self.current_centers += 1
