...
def getObjectDictionary(initializedCorpus, goldStandardCorpus):
	import codecs
	goldStandardSens = codecs.open(goldStandardCorpus, "r", encoding = "utf-8").readlines()
	initializedSens = codecs.open(initializedCorpus, "r", encoding = "utf-8").readlines()
	...
	for i in range(len(initializedSens)):
		...
		for k in range(len(initWordTags)):
			...
			if initWord != goldWord:
				...
				print(str(initWord.encode("utf-8")))
				print(str(initWord.encode("utf-8")))
			
			if initTag not in list(objects.keys()):
				objects[initTag] = {}
				objects[initTag][initTag] = []
				
			if correctTag not in list(objects[initTag].keys()):
